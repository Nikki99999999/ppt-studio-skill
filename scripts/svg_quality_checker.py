...
try:
    from layout_auditor import audit_file as _audit_layout_file
except ImportError:
    _audit_layout_file = None  # geometry layout audit will be skipped
...
class SVGQualityChecker:
    ...
    def check_file(self, svg_file: str, expected_format: str = None) -> Dict:
        ...
        try:
            ...
            if self._check_xml_well_formed(content, result):
                ...
                # 8. Check geometry-level layout contracts
                self._check_layout_geometry(svg_path, result)
    ...
    def _check_layout_geometry(self, svg_path: Path, result: Dict):
        """Run the annotation-aware geometry auditor.

        Hard errors require explicit contracts such as data-container or
        data-table-id/data-row/data-col. Generic overlap findings are warnings.
        """
        if _audit_layout_file is None:
            return
        try:
            report = _audit_layout_file(svg_path)
        except Exception as exc:
            result['warnings'].append(f"layout audit skipped: {exc}")
            return
        issues = report.get('issues', [])
        if not issues:
            return
        result['info']['layout_issues'] = len(issues)
        for issue in issues:
            message = (
                f"layout {issue.get('type')}: {issue.get('element')} — "
                f"{issue.get('message')}"
            )
    ...
    def _categorize_issue(self, error_msg: str) -> str:
        ...
        elif error_msg.startswith('layout '):
    ...
    def print_summary(self):
        ...
        if self.summary['errors'] > 0 or self.summary['warnings'] > 0:
            ...
            print(f"  5. Layout geometry: add data-container for text, data-table-id/data-row/data-col for tables, then fix reported bbox conflicts")
