...
Backend selection (`IMAGE_BACKEND` in `.env` or the current process environment):
  ...
  IMAGE_BACKEND=chatgpt-image-2 -> ChatGPT Image 2.0 via browser automation (Chinese text)
...
BACKEND_REGISTRY = {
    ...
    "chatgpt-image-2": {
        "module": "backend_chatgpt_image_2",
        "tier": "core",
        "label": "ChatGPT Image 2.0 (browser automation, accurate Chinese text)",
        "default_model": "chatgpt-image-2",
        "key_hint": "Chrome remote debugging + ChatGPT Plus/Max",
        "aliases": ["chatgpt", "gpt-image-2-web", "image-2"],
    },
    "minimax": {
...
def _resolve_backend() -> tuple[object, str]:
    ...
    """
    canonical = _resolve_backend_name()
    return _load_backend(canonical)


def _effective_model_for_backend(backend_name: str, cli_model: str | None) -> str:
    """Return the model name that will be used for strict requirement checks."""
    if cli_model:
        return cli_model.strip()
    if backend_name == "openai":
        return (os.environ.get("OPENAI_MODEL") or BACKEND_REGISTRY[backend_name]["default_model"]).strip()
    if backend_name == "chatgpt-image-2":
        return BACKEND_REGISTRY[backend_name]["default_model"]
    env_key = f"{backend_name.upper().replace('-', '_')}_MODEL"
    return (os.environ.get(env_key) or BACKEND_REGISTRY[backend_name]["default_model"]).strip()


def _enforce_model_requirement(requirement: str | None, backend_name: str, cli_model: str | None) -> None:
    """Fail closed when the caller requires a specific model family."""
    if not requirement:
        return
    normalized = requirement.strip().lower()
    model = _effective_model_for_backend(backend_name, cli_model)
    model_norm = model.lower()

    if normalized in {"image-2", "gpt-image-2", "image2"}:
        if backend_name == "chatgpt-image-2":
            return
        if backend_name == "openai" and model_norm.startswith("gpt-image-2"):
            return
        print(
            "Error: Image 2.0 is required, but the selected backend/model is not verified.\n"
            f"  backend: {backend_name}\n"
            f"  model:   {model or '(unknown)'}\n"
            "\n"
            "Allowed strict routes:\n"
            "  --backend chatgpt-image-2\n"
            "  --backend openai --model gpt-image-2\n"
        )
        sys.exit(1)

    print(f"Error: Unknown --require-model value: {requirement}")
    sys.exit(1)
...
def main() -> None:
    ...
    parser.add_argument(
        "--require-model", default=None,
        choices=["image-2", "gpt-image-2"],
        help="Fail closed unless the selected backend is verified for the required model family."
    )
    parser.add_argument(
    ...
    _enforce_model_requirement(args.require_model, backend_name, args.model)
    backend, backend_name = _load_backend(backend_name)
