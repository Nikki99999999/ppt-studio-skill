...
- **Content contract**: Read `content_manifest.json` together with `spec_lock.md`. The page title must match the approved Action Title, the page must express one `single_point`, and factual/data pages must render a visible `Source:` footer mapped to the evidence entry.
...
### 3.1 Layout Audit Contracts (Mandatory)

Every generated SVG must be understandable by `scripts/layout_auditor.py`. Add lightweight `data-*` annotations while drawing; these annotations do not affect rendering but make weaker model layout mistakes detectable.

**Element identity**:
- Give every major block an `id` or `data-id`: title areas, cards, panels, chart groups, image frames, callouts, tables, and table cells
- Add `data-role` to major blocks. Preferred roles: `container`, `card`, `panel`, `table`, `cell`, `chart`, `image`, `text-block`, `title`, `kpi`, `callout`
- Add `data-role="background"` or `data-role="decoration"` to intentional non-content layers so overlap checks ignore them
- Use `data-layout-ignore="true"` only for tiny decorative fragments, not for content

**Text containment**:
- When text belongs inside a visible box, card, panel, or table cell, set `data-container="<container-id>"` on the `<text>` element
- Add `data-max-lines` and `data-safe-padding` for constrained text. Example: `<text id="risk-copy" data-role="text-block" data-container="risk-card" data-max-lines="3" data-safe-padding="16" ...>`
- Manually wrap with `<tspan>` before the text reaches the container edge. Do not rely on the viewer to wrap text.

**Tables**:
- Mark every cell shape with `data-role="cell" data-table-id="<table-id>" data-row="<r>" data-col="<c>"`
- Put cell text in the matching cell container with `data-container="<cell-id>"`
- Keep row y/height and column x/width mathematically identical. If visual dividers need different strokes, draw them as separate `data-role="decoration"` lines.

**Overlap policy**:
- Intentional overlays are allowed only when semantically clear (for example, a label over an image). Add `data-role="decoration"` for purely visual layers or `data-layout-ignore="true"` for tiny non-content effects.
- Unintentional major-block overlap, text outside containers, table row/column drift, and canvas overflow must be fixed during the unified final quality gate before export.
