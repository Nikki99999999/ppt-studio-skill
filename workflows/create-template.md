...
Generate a complete set of reusable PPT layout templates for either the **private customer library** or the **public template library**.

> This workflow is for **library asset creation**, not project-level one-off customization. Customer uploads persist privately by default and become discoverable from `templates/customer_library/index.json`. Public promotion is a separate, explicitly approved step.

Read `../references/template-library-persistence.md` before processing a user-provided template.
...
| Persistence scope | Yes | Default `private-workspace`; use `public-library` only after explicit publication approval and privacy review |
...
If the reference source is HTML slides or a Frontend Slides-style design pack, read `../references/frontend-slides-design-pack.md` first. Extract style tokens, page rhythm, and layout archetypes, then rebuild a PPT-safe SVG template package. Do not copy HTML runtime code, JavaScript, CSS classes, animation, or `foreignObject` into final templates.

**Required outcome of Step 1**:

- The template is clearly positioned as a reusable private or public library template
...
```bash
mkdir -p "projects/<project>/templates/<template_id>"
```

> **Staging location**: Build and validate in the source project's template directory. After validation, register customer uploads into `templates/customer_library/`. Public templates go to `templates/layouts/` only after explicit publication approval.
>
...
## Step 6: Register Template in the Correct Library
...
python3 skills/presentation-studio/scripts/customer_template_library.py register \
  ...
  --source-file "<reference_source>" \
  --source-project "<project>"
```

This writes `provenance.json` and updates the ignored private index. Do not manually copy the raw customer source into the Skill directory.
