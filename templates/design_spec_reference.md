...
- **Footer area**: [Height and content description; every factual/data page reserves a visible `Source:` line mapped to `content_manifest.json`]
...
**Confirmed visual production strategy**: `[svg | user-provided | host-native | advanced-web | hybrid | placeholders]`

**Selected advanced model**: `[none | image-2 | nano-banana-pro | seedream-5-pro | seedance-2]`

| Filename | Dimensions | Ratio | Purpose | Type | Media | Model | Route | Status | Generation Description |
| -------- | --------- | ----- | ------- | ---- | ----- | ----- | ----- | ------ | --------------------- |
| cover_bg.png | {canvas_info['dimensions']} | [ratio] | Cover background | [Background/Photography/Illustration/Diagram/Decorative] | [image/keyframe/video] | [none/host-selected/image-2/nano-banana-pro/seedream-5-pro/seedance-2] | [local-svg/host-native/web] | [Pending/Existing/Placeholder] | [AI generation prompt] |
...
- The user's model choice is a lock. Named advanced models use only the corresponding verified web route in `templates/image_models/model_catalog.json`.
- Host-native output must remain labeled `host-selected`; never relabel it as a named model without explicit tool attestation.
- Seedance rows use `video` or `keyframe` media. Standard PPTX delivery must not claim embedded-video support unless the exporter verifies it.
...
- **Title**: [Action Title: a conclusion sentence, not a topic label]
- **Single point**: [The one claim this page must land]
- **Source footer**: [Source mapped from `content_manifest.json`, or `None — no factual claim`]
