...
- route: web
- web_entry: https://chatgpt.com/images
- model_verification: explicit-ui
- fallback_policy: none-without-user-approval
- watchdog_seconds: 300
- poll_interval_seconds: 25

> Fill this block from the user's confirmed visual production strategy. For pure SVG use `selected_model: none`, `route: local-svg`, and `model_verification: not-required`. For host-native use `selected_model: host-selected`; never insert a named model unless the tool or web UI explicitly verifies it. Remove `web_entry` for non-web routes.
